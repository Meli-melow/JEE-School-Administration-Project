...
<<<<<<< Updated upstream
=======
    public List<Student> daoTeacherSeeStudents(String schoolYear) {
        ...
        try {
            ...
            try {
                ...
                throw new DeleteCascadeException("Could not propagate modifications correctly. Teacher could not see their students " + fex);
            }
            em.getTransaction().commit();
        } catch (Throwable ex) {
            throw new ExceptionInInitializerError("EntityManagerFactory initialisation failed : " + ex);
